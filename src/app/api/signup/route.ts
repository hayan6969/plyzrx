...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (existingEmailResult.documents.length > 0) {
      const existingUser = existingEmailResult
        .documents[0] as unknown as Signedupusers;
        ...
        return NextResponse.json(
          {
            success: false,
            message:
              "An account with this email is already registered and verified. Please sign in instead.",
          },
          { status: 409 }
        );
      } else {
  ...
  try {
    ...
    // Track signup with FirstPromoter
    const cookieStore = cookies();
    const tid = await getFirstPromoterTrackingId(cookieStore);
    await trackFirstPromoterSignup(email, userDoc.$id, tid);

    return NextResponse.json({
      success: true,
      message:
        "Please check your email for OTP verification to complete your account creation.",
      username: username,
